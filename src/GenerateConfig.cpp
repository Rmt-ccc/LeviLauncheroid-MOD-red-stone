...
int main(int argc, char **argv) {
  if (argc != 2) {
    std::cerr << "usage: redstone_tweaks_mod_config_gen <mod-package-dir>\n";
    return 2;
  }

  const std::filesystem::path packageDir(argv[1]);
  const auto configDir = packageDir / "config";
  pl::config::ConfigFile<redstonetweaks::RedstoneTweaksConfig> config(
      redstonetweaks::RedstoneTweaksConfig{}, configDir / "config.json",
