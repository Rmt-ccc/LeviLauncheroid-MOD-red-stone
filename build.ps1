...
# 公式サンプルと違い、このリポジトリは単独リポジトリなので、
# third_party/preloader-android (submodule) を既定値にする。
if ([string]::IsNullOrWhiteSpace($PreloaderRoot)) {
    $PreloaderRoot = Join-Path $SourceDir "third_party\preloader-android"
}
...
Invoke-Checked cmake `
    --build $HostBuildDir `
    --target redstone_tweaks_mod_config_gen

$GeneratorCandidates = @(
    (Join-Path $HostBuildDir "redstone_tweaks_mod_config_gen.exe"),
    (Join-Path $HostBuildDir "redstone_tweaks_mod_config_gen")
)
...
Invoke-Checked cmake `
    ...
    "-DCMAKE_BUILD_TYPE=$BuildType" `
    "-DPRELOADER_ANDROID_ROOT=$PreloaderRoot" `
    "-DLL_MOD_LINK_PRELOADER=$LinkPreloader"

Invoke-Checked cmake `
    --build $AndroidBuildDir `
    --target redstone_tweaks_mod

Remove-DirectoryIfExists $DistDir
$PackageDir = Join-Path $DistDir "RedstoneTweaksMod"
$ConfigDir = Join-Path $PackageDir "config"
New-Item -ItemType Directory -Force -Path $ConfigDir | Out-Null

$LibraryPath = Join-Path $AndroidBuildDir "out\$Abi\libredstone_tweaks_mod.so"
...
Copy-Item -LiteralPath $LibraryPath `
    -Destination (Join-Path $PackageDir "libredstone_tweaks_mod.so")

Invoke-Checked $Generator $PackageDir

$ArchivePath = Join-Path $DistDir "RedstoneTweaksMod.levipack"
$TempZipPath = Join-Path $DistDir "RedstoneTweaksMod.zip"
...
Write-Host "Built RedstoneTweaksMod:"
